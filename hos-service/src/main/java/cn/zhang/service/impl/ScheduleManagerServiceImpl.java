...
public class ScheduleManagerServiceImpl implements ScheduleManagerService {
	...
	public PageBean<ScheduleResult> getPageBean(QueryVO queryVO) {
		if (queryVO != null) {
			...
			PageBean<ScheduleResult> pageBean = new PageBean(queryVO.getCurrentPage(), queryVO.getPageSize());
	...
	public List<TSchedule> listScheduleByDoctorId(Long id) {
		...
		example.setOrderByClause(OrderConsts.SCHEDULE_ORDER_BY_WORK_DATE + "," + OrderConsts.SCHEDULE_ORDER_BY_WORK_TIME);
	...
	public List<SourceNumber> listSourceNumberByScheduleId(Long scheduleId) throws ParseException {
		...
		if(!schedule.getWorkTime()){
			return listSourceNumber(startWork, TimeConsts.DEPARTMENT_OFF_DUTY_TIME_MORNING, timeInterval, date, scheduleId);
		} else {
			return listSourceNumber(TimeConsts.DEPARTMENT_WORKING_HOURS_AFTERNOON, endWork, timeInterval, date, scheduleId);
		}
	}
