...
<script type="text/javascript">
	...
	function deleteFeedback(id) {
		...
		$.post("${pageContext.request.contextPath }/feedbackManager/deleteFeedback.action",
			{"id" : id},
			function (data) {
				if (data.status == 200){
					alert("删除成功");
	...
	function deleteFeedbacks(){
		...
		$.post(
			"${pageContext.request.contextPath}/feedbackManager/deleteFeedbacks.action",
			{"ids":str},
			function(data){
				if(data.status == 200){
					alert("删除成功!");
...
<body class="hold-transition skin-purple sidebar-mini">
		...
		<section class="content-header">
			...
			<ol class="breadcrumb">
				...
				<li><a href="all-travellog-review-list.html">留言管理</a></li>
					...
					<div class="table-box">
							...
							<div class="form-group form-inline">
								<div class="btn-group">
									...
									<button type="button" class="btn btn-default" title="刷新"
										onclick="window.location.href='${pageContext.request.contextPath }/feedbackManager/list.html'">
						...
						<div class="box-tools pull-right">
							<div class="has-feedback">
								<form id="myForm"
									action="${pageContext.request.contextPath}/feedbackManager/list.html"
									method="post">
									<input type="hidden" id="currentPageInput" name="currentPage"
										...
										id="searcher" name="userTel" value="${param.userTel}" />
								</form>
						...
						<table id="dataList"
							...
							<tbody>
								<c:forEach items="${pageBean.list }" var="feedback">
									<tr>
										<td><input name="ids" type="checkbox" value="${feedback.id}"></td>
										<input type="hidden" name="id" value="${feedback.id}">
										<td>${feedback.name }</td>
										<td>${feedback.phone }</td>
										<td >
											<div class="wrap1" title="${feedback.content }">
												${feedback.content }
											</div>
										</td>
										<td>${feedback.created_x }</td>

										<td class="text-center">
											<button type="button" class="btn bg-olive btn-xs"
												onclick='deleteFeedback(${feedback.id})'>删除</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>
