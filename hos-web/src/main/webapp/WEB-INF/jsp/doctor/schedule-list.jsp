...
<script type="text/javascript">
	...
	/* function deleteUser(){
		...
		$.post(
			"${pageContext.request.contextPath}/scheduleManager/deleteSchedules.action",
			{"ids":str},
			function(data){
				if(data.status == 200){
					alert("删除成功!");
...
<body class="hold-transition skin-purple sidebar-mini">
		...
		<section class="content-header">
			<h1>
				排班查询 <small>排班列表</small>
			</h1>
			<ol class="breadcrumb">
				<li><a href="all-admin-index.html"><i
						class="fa fa-dashboard"></i> 首页</a></li>
				<li><a href="all-travellog-review-list.html">排班查询</a></li>
					...
					<div class="table-box">
							...
							<div class="form-group form-inline">
								<div class="btn-group">
									<button type="button" class="btn btn-default" title="刷新"
										onclick="window.location.href='${pageContext.request.contextPath}/doctorSchedule/list.html?doctorId=${doctorUser.id}'">
						...
						<div class="box-tools pull-right">
							<div class="has-feedback">
								<form id="myForm"
									action="${pageContext.request.contextPath}/doctorSchedule/list.html"
									method="post">
...
<!-- 									<th class="text-center">操作</th> -->
							...
							<tbody>
								<c:forEach items="${pageBean.list }" var="schedule">
									<tr>
<%-- 										<td><input name="ids" type="checkbox" value="${schedule.id}"></td> --%>
										<%-- <td>${schedule.doctorRealname }</td>
										<td>${schedule.doctorTel }</td>
										<td>${schedule.depName }</td> --%>
										<td>${schedule.workDate_x }</td>
										<td>${schedule.workTime_x }</td>
										<td>${schedule.timeInterval }/分钟</td>

										<%-- <td class="text-center">
											<button type="button" class="btn bg-olive btn-xs"
												onclick='location.href="${pageContext.request.contextPath }/scheduleManager/toEdit.html?id=${schedule.id}"'>编辑</button>
										</td> --%>
									</tr>
								</c:forEach>
							</tbody>
